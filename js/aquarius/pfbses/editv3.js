...
console.log('SaveService invoked')
		$.ajax({
...
saveDevice = _=>{
	$.ajax({
		url:'/pfbses/savedevice',
		data:{
			device_id:$('#devices').val(),fb_id:$('#no_fb').text()
		},
		type:'post'
	})
	.done(res=>{
		console.log('Success save device',res);
	})
	.fail(err=>{
		console.log('Error save device',err);
	});
}
$('#btnservicesave').click(function(){
	console.log('fb service saved',$('#servicecategories').val(),$('#servicedescription').val());
	console.log("Product selected",$('#products').val());
	switch ($('#products').val()){
	...
	$.ajax({
		...
		$.ajax({
			...
			$.ajax({
				...
				console.log('Tidak dapat mengupdate tbl clientsite',err);
			});
...
$('#products').change(function(){
	...
	switch ($(this).val()){
		case '0':
			...
			$('.pdevice').hide();
			$('.pvas').hide();
			$('#dservicedescription').show();
		break;
		case '1':
			$('.pservice').hide();
			$('.pdevice').show();
			$('.pvas').hide();
			$('#dservicedescription').hide();
		break;
		case '2':
			$('.pservice').hide();
			$('.pdevice').hide();
...
$('#dAddService').on('shown.bs.modal',e=>{
	$('#products').val(0);
	$('.pservice').show();
	$('.pdevice').hide();
	$('.pvas').hide();
