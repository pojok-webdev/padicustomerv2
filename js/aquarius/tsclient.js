(function($){
	...
	$('#tClient').on('click','tr .btnShowVAS',function(){
		var id=$(this).stairUp({level:4}).attr('rowid');
		$('#tClient tbody tr').removeClass('selected');
		$(this).stairUp({level:4}).addClass('selected');
		$.ajax({
			url:'/clients/getvas/'+id,
			type:'get',
			dataType:'json'
		})
		.done(function(res){
			console.log('VAS',res);
			$("#tVAS tbody").empty();
			$.each(res,function(id,vas){
				$("#vasclientname").html(vas.client)
				console.log('vas',vas);
				text = '<tr>';
				text+= '<td>'+vas.name+'</td>';
				text+= '<td class="alias">'+vas.description+'</td>';
				text+= '</tr>';
				$("#tVAS tbody").append(text)
			})
			$('#dShowVAS').modal();
		});
