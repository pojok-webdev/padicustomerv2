...
//		"aoColumnDefs":[{"bSearchable":true,"bVisible":false,"aTargets":[4]}]
	...
	$('#tClient').on('click','tbody tr .btneditalias',function(){
		$('#tClient tbody tr').removeClass('selected');
		$(this).stairUp({level:4}).addClass('selected');
		var myid = $(this).stairUp({level:4}).attr("myid"),
		name = $(this).stairUp({level:4}).find(".name").html(),
		alias = $(this).stairUp({level:4}).find(".alias").html();
		clientname = name+' ('+alias+')',
		console.log('MyId',myid,alias);
		$.ajax({
			url:'/clients/getalias',
			data:{id:myid},
			type:'post',
			dataType:'json'
		})
		.done(function(res){
			...
			$('#setAliasModalLabel').html(' '+clientname);
			$('#nametext').val(name);
			$('#aliastext').val(alias);
			$('#dSetAlias').modal();
		})
	...
	$('#btnSaveAlias').click(function(){
		$.ajax({
			url:'/clients/savealias',
			data:{
				id:$('#tClient tbody tr.selected').attr('myid'),
				alias:$('#aliastext').val(),
				name:$('#nametext').val()
			},
			type:'post',
		...
		.done(function(res){
			$('#tClient tbody tr.selected').find('.alias').html($('#aliastext').val());
			$('#tClient tbody tr.selected').find('.name').html($('#nametext').val());
			$('#dSetAlias').modal('hide');
		})
		.fail(function(err){
			console.log('Err',err);
		});
