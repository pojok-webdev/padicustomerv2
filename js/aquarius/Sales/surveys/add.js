	...
	$('#survey_save').click(function () {
		...
		if(tsbranchok && salesbranchok){		
			if ($(this).checkEmpty({className: "emptycheck"})) {
				console.log('SURVEYREQUEST:',$('.surveyrequest').attr('keyval'));
				if($("#clientcategory").val()==='0'){
					alert("Kategori Pelanggan harus dipilih");
				}else{
				...
				$.ajax({
					...
					console.log('CLIENTSITE',$('.clientsite').attr('keyval'));
					...
					$.ajax({
						...
						console.log('surveysite',$('.surveysite').attr('keyval'));
						$.ajax({
							...
							status = 'test', 
							url = '/surveys/edit/'+survey_site_id,
							createuser = $('#createuser').val();
							console.log("Client Category",$("#clientcategory").val());
							/*if($("#clientcategory").val()==='0'){
								alert("Kategori Pelanggan harus dipilih");
							}else{*/
								$.ajax({
									...
									console.log("Error clients update",err);
									...
									console.log("clients update success "+data);
										...
										$.ajax({
											...
											$.ajax({
												...
												data:{
													period:"0",
													perioddetail:fullDate,
													subject:"Pengajuan Survey",
													content:'Pengajuan Survey',
