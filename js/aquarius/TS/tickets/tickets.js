...
$("#tbl_ticket").on("click","tr .editUpstream",function () {
	var selected = $(this).stairUp({level:4}),
		...
		$("#addUpstreamTitle").html("Edit Gamas ");
		$("#addUpstreamID").html(selected.find(".kdticket").html())
		$.ajax({
		...
		.done(function(ticket){
			...
			.done(function(res){
				...
				for(var i=0;i<res.length;i++){
					...
					str = '<tr trid="'+res[i].id+'">'
					str+= '<td>'+res[i].clientname+'</td>'
					str+= '<td>'+res[i].address+'</td>'
					str+= '<td>'+res[i].email+'</td>'
					str+= '<td>'+res[i].picname+'</td>'
					str+= '<td>'+res[i].picphone+'</td>'
					str+= '<td class="removeExistingClient">Hapus</td>'
					str+= '</tr>';
...
$('#tMailRecipients').on('click',' tbody tr .picremover',function(){
	console.log("picremover invoked");
	$(this).parent().parent().remove();
});
$('#dPersonalMailSetting').on('shown.bs.modal', function () {
	console.log('Active tab',$('.nav-tabs li.active')[1].innerText);
	switch($('.nav-tabs li.active')[1].innerText){
		case 'Preview':
			$('#btnprevttab').attr("disabled",false)
			$('#btnnexttab').attr("disabled",true)
		break;
		case 'Parameters':
			validateTabOne(valid=>{
				if(valid){
					$('.nav-tabs a[href="#personalhome"]').tab('show')
				}else{
					$('.nav-tabs a[href="#addTicket"]').tab('show')
					//alert("Data Tab 1 kurang lengkap");
				}
			});			
		break;
		case 'Add Ticket':
			$('#btnprevttab').attr("disabled",true)
