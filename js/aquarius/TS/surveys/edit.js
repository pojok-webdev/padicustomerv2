(function($){
	...
	$(".choosePackage").click(function(){
		$.ajax({
			url:"/surveypackagedetails/jsonbypackageid/"+$(this).attr("thisid"),
			dataType:"json",
		}).done(function(data){
			console.log(data);
			$.each(data,function(detail,device){
				$.ajax({
					url:'/survey_devices/save',
					...
					$(".device tbody tr td.editable").click(function(){
						...
						$.ajax({
							url:"/survey_devices/jsonbyid/"+$(this).stairUp({level:1}).attr("thisid"),
							type:"get",
							dataType:"json",
						}).done(function(data){
							$("#device_type").selectopt(data["devicetype_id"]);
							$("#device_name").populateCombo({
								url:"/devices/getdevice/"+data["devicetype_id"],
	...
	$(".survey_save").click(function(){
		...
		var thisstatus='3';
		var result;
		if(($("#site_execution_date").val()=="")||$("#site_execution_date").val()=="//"){
			alert("Tanggal Eksekusi harus diisi");
		}else{
			console.log('Survey Saved',$("#site_execution_date").val());
			...
			if($('#longresume').val()==='' && surveydatehaschange($('#workplace').attr('survey_id'))){
				...
				$.ajax({
					...
					console.log('PRINT KEYVAL',$('.surveysite').attr('keyval'));
					$.ajax({
						...
						console.log('Description val',$('#description').text());
						$.ajax({
							url:'/surveys/updatedescription',
							data:{
								id:$('#workplace').attr('site_id'),
								description:$('#site_descriptions').text()
							},
							type:'post'
						})
						.done(function(res){
							console.log('successs save site description',res);
						})
						.fail(function(err){
							console.log('Failed save site description',err)
						});
						$('#surveyresult').html(thiselement.text());
						var createuser = $('#createuser').val(),
						clientname = $("#client_name").val(),
						status = thiselement.text(),
						url = "/surveys/edit/"+$('#workplace').attr('site_id'),
						reporturl = "/surveys/showreport/"+$('#workplace').attr('site_id');
						...
						console.log("Error update survey_site",err);
					});
				})
				.fail(function(err){
