...
class Maintenancereport extends CI_Model{
    ...
    function get_maintenance_report(){
        $sql = "select a.id,b.id scheduleid,date_format(a.maintenancedate,'%d/%m/%Y')maintenancedate,a.createdate sqldate,";
        $sql.= "case b.period_type ";
        ...
        /*$sql.= "left outer join ";
        $sql.= " (";
        $sql.= "  select maintenancereport_id,image,max(createdate)createdate ";
        $sql.= "  from maintenancereport_images where imagetype='1' group by maintenancereport_id,image";
        $sql.= " ) e ";
        ...
        $sql.= "a.status,a.createdate,d.name,a.clientrequest ";
        $ci = & get_instance();
        $que = $ci->db->query($sql);
        return $que->result();
    }
    function get_maintenance_report_(){
...
//        return $que->result()[0]->name;
    ...
    function getalleos(){
        $sql = "select b.id,b.username name from groups_users a left outer join users b on b.id=a.user_id ";
        $sql.= "left outer join groups c on c.id=b.group_id ";
        $ci = & get_instance();
        $que = $ci->db->query($sql);
        return $que->result();
    ...
    function getmaintenancescheduleclientrequest($maintenancereport_id){
        $sql = "select clientrequest from maintenancereports a ";
        $sql.= "left outer join maintenances b on b.id=a.maintenance_id ";
        $sql.= "where a.id='".$maintenancereport_id."'";
        $ci = & get_instance();
        $que = $ci->db->query($sql);
        return $que->result()[0]->clientrequest;
    }
