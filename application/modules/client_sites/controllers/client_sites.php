...
echo $this->pclient_site->setactive($params);
}
	...
	function addsurveysite(){
		$params = $this->input->post();
		echo $this->client_site->add($params);
	}
	...
	function feedData(){
		...
		$objs = $this->client_site->populate($client_id);
		$rows = array();
		foreach($objs as $obj){
			$vals = array();
	...
	function index(){
		...
		switch($this->session->userdata["role"]){
			...
			case 'Sales':
				...
				break;
			case 'TS':
				$this->load->view('TS/clients/client_sites20210726',$this->data);
				break;
			case 'Accounting':
	...
	function populatebyclient(){
		...
		$arr = array();
		foreach($this->client_site->get_combo_data($id) as $key=>$val){
	...
	function save(){
		$params = $this->input->post();
		$this->load->model('pclient_site');
		echo json_encode($this->pclient_site->save($params));
		/*$params = $this->input->post();
		$obj = new Client_site();
	...
	function update(){
		...
		//$obj = new Client_site();
		//$obj->where('id',$params['id'])->update($params);
		return $this->pclient_site->update($params);
		//return $obj->check_last_query();
	}
	...
	function view_devices(){
		...
		$this->data['menuFeed'] = 'clientSite';
		$client_id = $this->uri->segment(3);
		$objs = $this->client_site->populatebyid($client_id);
		if($this->uri->total_segments()==3){
			$this->data['clientname'] = $objs->client->name;
		}else{
			$this->data['clientname'] = 'Semua';
