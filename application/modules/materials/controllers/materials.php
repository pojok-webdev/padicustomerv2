...
class Materials extends CI_Controller{
	...
	function __construct(){
		...
		$this->preference = $this->common->get_preferences();
		$this->lang->load('padi',$this->setting['language']);
	}
	function show_materials(){
		$obj = new Material();
		$this->common->show_object($obj, 'materials','materials');
	}
	...
	function getmaterial(){
		...
		$arr = array();
		foreach($this->material->get_combo_data($materialtype) as $key=>$val){
	...
	function get_name_by_parent(){
		...
		$arr = array();
		foreach($this->material->get_name_by_parent($parent) as $key=>$val){
			array_push($arr,'"' . $key . '":"' . $val . '"');
		}
		$out = implode(',',$arr);
		$out = '{' . $out . '}';
