...
class Disconnections extends CI_Controller{
	...
	function add(){
		$this->data['obj'] = $this->client->get_obj_by_id($this->uri->segment(3));
	...
	function add_lookup(){
		...
		$this->data['objs']=$this->client->populate(array('9','1','3','0'),'1');
		$this->data['return_page']='adm/disconnection_add/';
		$this->data['menuFeed']='disconnection';
		$this->load->view('Sales/disconnections/add_disconnection_lookup',$this->data);
	}
	...
	function entryreport(){
		$this->check_login();
		$obj = $this->disconnection->get_obj_by_id($this->uri->segment(3));
		...
		$data = array(
			...
			"antennas"=>$this->disconnection->get_antennas_by_id($this->uri->segment(3)),
			"apwifis"=>$this->disconnection->get_apwifi_by_id($this->uri->segment(3)),
			"devices"=>$this->disconnection->get_devices_by_id($this->uri->segment(3)),
			"pccards"=>$this->disconnection->get_pccards_by_id($this->uri->segment(3)),
			"pics"=>$this->disconnection->get_pics_by_id($this->uri->segment(3)),
			"routers"=>$this->disconnection->get_routers_by_id($this->uri->segment(3)),
			"switches"=>$this->disconnection->get_switches_by_id($this->uri->segment(3)),
			"wireless_radios"=>$this->disconnection->get_wireless_radios_by_id($this->uri->segment(3)),
			"dtype"=>$dtype
		);
	...
	function index(){
		...
		$this->data['menuFeed']='disconnection';
		$this->data['objs'] = $this->disconnection->populate($status);
		$this->data['hours'] = $this->common->gethours();
		$this->data['minutes'] = $this->common->getminutes();
	...
	function edit(){
		$this->check_login();
		$this->data['obj'] = $this->disconnection->get_obj_by_id($this->uri->segment(3));
		$this->data['hours'] = $this->common->gethours();
		$this->data['minutes'] = $this->common->getminutes();
		$this->data['officers']=$this->user->get_user_by_group('TS');
		$this->data['menuFeed']='disconnection';
...
//			$this->load->view('TS/disconnections/edit',$this->data);
	...
	function filter(){
		...
		switch($this->uri->segment(3)){
			case "executed":
				$this->data['objs'] = $this->disconnection->populate('1');
				break;
			case "notexecuted":
				$this->data['objs'] = $this->disconnection->populate('0');
				break;
		}
		switch($this->session->userdata["role"]){
			case 'Sales':
	...
	function reminder(){
		...
		foreach($objs as $obj){
			...
			if($obj->disconnectiontype=='2'){
				$mdatetime = $this->common->longsql_to_datepart($obj->reactivationdate);
	...
	function save(){
		$params = $this->input->post();
		$this->load->model('pdisconnection');
		switch($params['disconnectiontype']){
			case '3':
			echo($this->pdisconnection->savepermanent($params));
			break;
			case '2':
			echo($this->pdisconnection->savetemporer($params));
			break;
		}
		/*
		print_r($params);
		...
		$obj->save();
		$this->app_log->create_log($obj->check_last_query());
		echo $this->db->insert_id();*/
	}
	function sendemail(){
		$params = $this->input->post();
	...
	function feedData(){
		$objs = $this->disconnection->populate();
		$rows = array();
		foreach($objs as $obj){
			$vals = array();
			switch($obj->status){
			...
			array_push($vals,'"finishdate":"'.$this->common->sql_to_human_datetime($obj->finishdate).'"');
			array_push($vals,'"reactivationdate":"'.$this->common->sql_to_human_datetime($obj->reactivationdate).'"');
