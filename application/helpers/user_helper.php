...
function getsubordinates($user_id,&$arr){
	$user = new Puser();
	$res = $user->getsubordinates($user_id);
...
function user_in_group($user_id,$group_id){
	$sql = 'select * from groups_users where group_id='.$group_id.' and user_id='.$user_id.' ';
	$ci = & get_instance();
	$que = $ci->db->query($sql);
	$res = $que->result();
