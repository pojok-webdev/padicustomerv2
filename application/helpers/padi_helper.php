...
function padi_addprefix($prefix = '0',$maxlength=2,$str){
	for($c=strlen($str);$c<$maxlength;$c++){
		$str = $prefix.$str;
	}
	return $str;
}
function padi_getvaluesarray($startval = 0,$endval = 100){
	$out = array();
	for($c=$startval;$c<=$endval;$c++){
		$out[$c]=padi_addprefix('0',2,$c);
	}
	return $out;
}
