							...
							<div class="row-form clearfix" id="client_site_textbox_div">
								...
								<div class="span8">
									<select id="complain" name="complaint" class="inp_ticket2" type="select">
										...
										<option>VOIP</option>
										<option>WLL Problem</option>
										<option>Konsultasi</option>
										<option>Problem Televisi</option>
				...
				success:function(clnts){
					...
					$('#autocomplete-pelanggan').autocomplete({
						...
						onSelect: function(suggestion) {
							...
							$.ajax({
								url:'/clients/get_sites',
								type:'post',
								data:{id:suggestion.data},
								dataType:'json',
								success:function(sites){
console.log('sites',Object.keys(sites).length);
									narasi = 'Nama Pelanggan ini memiliki <strong>'+Object.keys(sites).length+' sites </strong>'
									narasi+= 'Silakan dipilih dan dipastikan dahulu '
									narasi+= 'Jika tidak menemukan nama/alamat, coba menggunakan nama lain '
									narasi+= 'atau menghubungi <strong>Rendy</strong>'
									console.log('NARASI',narasi);
$('#narasi').html(narasi);
									$('#csites').empty();
									$.each(sites,function(x,y){
										$('#csites').append('<option value='+x+'>'+y.alamat+'</option>');
										$.ajax({
											url:'/clients/get_services',
											type:'post',
											data:{id:x},
											dataType:'json',
			...
			$('#btn_saveticket').click(function(){
				...
				if(!$('#client_id').val()){
					alert('Pelanggan harus memilih dari list yang ada');
				}else
				{if($("#complain option:selected").text()==="Pilihlah"){
					alert("Keluhan harus diisi");
				}else{
$('#btn_saveticket').prop('disabled',true)
				...
				$.ajax({
					url: '/tickets/save',
					data: JSON.parse('{' + $(".inp_ticket2").attr("keyval") + ',"client_id":"'+$('#client_id').val()+'","client_site_id":"'+$('#csites').val()+'","clientname":"' + $("#autocomplete-"+optreq).val() + '","downtimestart":"1900-01-01 00:00:00","downtimeend":"1900-01-01 00:00:00"}'),
					type: 'post',
					async: false
				}).fail(function (err) {
					console.log('Tidak dapat menyimpan data, silakan hubungi Developer',err);
				}).done(function (data) {
					$.ajax({
...
$('#autocomplete-pelanggan').click(function(){
$(this).select();
});
$('#autocomplete-pelanggan').keyup(function(){
console.log('must be changed');
if(!$('#autocomplete-pelanggan').val()){
