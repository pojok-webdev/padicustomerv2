<html>
	...
	<link rel="stylesheet" href="/asset/report/bootstrap-3.3.6.min.css" />
		...
		<link rel="stylesheet" href="/css/rpt/periodtop5.css">
	...
	<body>
		...
		Laporan Top 5 Main Root Cause
		</div>
		<div class='subject' id='sub'><?php echo $date1 . ' - ' . $date2;?> (<?php echo $total;?>)</div>
		<div class='filter'>
		...
		<div class='filter'>
			...
			foreach($arrbranch as $br){
				...
				echo '<label class="checkbox-inline suspect-branches"><input class="cbranch" type="checkbox" value="'.$br.'" '. $checked .' >';
				echo getbranch($br) . " ";
				echo '</label>';
			}
			?>
				...
				<tbody>					
					...
					<?php foreach($tickets as $ticket){?>
                        ...
                        <tr trid="<?php echo $ticket->id;?>">
                            ...
                            <td>
                            <span class="causedetail"><?php echo '<a href="/rpt/getticketsbymainroot/'.$dateselected1.'/'.$dateselected2.'/'.$branches.'/'.$ticket->id.'">'. $ticket->cnt;?></span>
                            </td>
						</tr>
			...
			(function($){
				...
				$("#btnfilter").click(function(){
					...
					if($("#reportdate1").val().trim()!==""){
						...
						window.location.href = "/rpt/periodtop5/"+dt1+"/"+mn1+"/"+yr1+"/"+dt2+"/"+mn2+"/"+yr2+"/"+branch+"/"+$('#numbertoshow').val();
					}
				})
