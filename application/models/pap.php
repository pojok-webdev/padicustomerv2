...
class Pap extends CI_Model{
    ...
    function getclientsites($ap_id){
        $sql = "select a.id,d.name,c.address from aps a ";
        $sql.= "left outer join client_sites_aps b on b.ap_id=a.id ";
        $sql.= "left outer join client_sites c on c.id=b.client_site_id ";
        $sql.= "left outer join clients d on d.id=c.client_id ";
        $sql.= "where a.id = " . $ap_id . " ";
        $ci = & get_instance();
        $que = $ci->db->query($sql);
        $res = $que->result();
        $tot = $que->num_rows();
    ...
    function get_combo_data_plus($first_row = '',$bts_id=null){
        ...
        $comment.= "************************************ ";
        $sql = "select a.id,a.namex,a.ipaddr,a.description,a.user_name,b.name btsname from aps a ";
