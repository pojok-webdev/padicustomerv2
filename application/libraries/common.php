...
class Common {
	...
	function grantElement($owner='everyone',$executor='everyone'){
		...
		if($executor=='group'){
			if($this->group->get_groupname($this->obj->ionuser->group_id) == $owner){
				return '' . $owner;
			}
			return 'disabled="disabled"';
		}
//		return 'disabled="disabled"';
	...
	function hasAncestor($decessor_id){
		$puser = new Puser();
		return $puser->hasAncestor($decessor_id);
/*		$obj = new User();
	...
	function IsDecessor($decessor_id,$ancestor_id){
		...
		$supervisoridarray = $puser->getsupervisoridarray($decessor_id);
		if($decessor->supervisor->id==$ancestor_id){
			return true;
		}else{
			if($this->hasAncestor($supervisoridarray)){
...
//				return true;
	...
	function check_messages(){
		...
		$message->where('recipient',$this->session->userdata['id'])->or_where('recipient_group',$this->user->get_group_id($this->session->userdata['id']))->where('hasread','0')->get();
	...
	function show_object($class,$view_data,$pagination,$column='name'){
		...
		$data = array(
			...
			'page_count'=>ceil($total/$per_page),'uri'=>$uri,'alertcount'=>$this->common->check_messages(),
		);
