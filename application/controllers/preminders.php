...
class Preminders extends CI_Controller{
    ...
    function sendmail($clientname,$message,$createuser,$baseurl,$id,$subject='padiApp --test notifikasi',$to='puji@padi.net.id'){
    ...
    function fbupdate(){
        $reminder = new Preminder();
        $rows = $reminder->getfbupdates();
        foreach($rows as $row){
            $msg = $row->description . ' ' . $row->bdescription. ' ' . $row->bname . ' ' . $row->role;
            $this->sendmail($row->name,'Telah terjadi update FB <br />'.$msg,$this->session->userdata('username'),'/',1,'Update FB : '.$row->name,'puji@padi.net.id');
            $reminder->setfbsent($row->id);
        }
    }
    function fbchange(){
        $reminder = new Preminder();
        $rows = $reminder->getfbchange();
        $str = '<style>';
        $str.= 'table th{';
            $str.= 'border: black solid 1px;';
            $str.= 'margin: 0;';
            $str.= '}';
        $str.= 'table td{';
            $str.= 'border: black solid 1px;';
        ...
        $str.= '}';
        $str.= '</style>';
        $str.= 'Berikut adalah perubahan pada FB <br />';
        $str.= '<table>';
        $str.= '<thead>';
        $str.= '<tr>';
        $str.= '<th>No.</th>';
        $str.= '<th>No FB</th>';
        $str.= '<th>Field Yang Berubah</th>';
        $str.= '<th>Sebelum Perubahan</th>';
        $str.= '<th>Sesudah Perubahan</th>';
        $str.= '<th>Keterangan</th>';
        $str.= '<th>Waktu Perubahan</th>';
        $str.= '</tr>';
        $str.= '</thead>';
        $str.= '<tbody>';
        $c = 1;
        foreach($rows as $row){
            $str.= '<tr>';
            $str.= '<td>'.$c++.'</td>';
            $str.= '<td><a href="/pfbses/edit/'.$row->nofb.'">'.$row->nofb.'</a></td>';
            $str.= '<td>'.$row->colname.'</td>';
            $str.= '<td>'.$row->valbefore.'</td>';
            $str.= '<td>'.$row->valafter.'</td>';
            $str.= '<td>'.$row->description.'</td>';
            $str.= '<td>'.$row->createdate.'</td>';
            $str.= '</tr>';
            $reminder->setfbsent($row->id);
        }
        $str.= '</tbody>';
        $str.= '</table>';
        $this->sendmail('Nama Klien',$str,'puji','/',1,'[padiApp] Perubahan FB','ketut@padi.net.id');
    }
}