...
class Paps extends CI_Controller{
	...
	function add(){
		...
		echo $this->pap->add($params);
	}
	function clients(){
		$ap_id = $this->uri->segment(3);
		$objs = $this->pap->getclientsites($ap_id);
		$data = array(
			'menuFeed'=>'AP',
			'objs'=>$objs['res'],
			'tot'=>$objs['tot']
		);
		$this->load->view("adm/aps/apsclientsites",$data);
	}	
	function getJSON(){
		$id = $this->uri->segment(3);
		$aps = $this->pap->getbyid($id);
		echo '{"btstower_id":"'.$aps->btstower_id.'","name":"'.$aps->name.'","ipaddr":"'.$aps->ipaddr.'","description":"'.$aps->description.'"}';
	}
	function getplus(){
		$arr = array();
		foreach($this->pap->get_combo_data_plus() as $key=>$val){
	...
	function get(){
		$arr = array();
		foreach($this->pap->get_combo_data() as $key=>$val){
			array_push($arr, '"' . $key . '":"' . $val . '"');
		}
		$out = implode(',',$arr);
		$out = '{' . $out . '}';
...
//					$obj->where('id',$uri['id'])->get();
	...
	function get_name_by_bts(){
		...
		$arr = array();
		$this->load->model('pap');
		foreach($this->pap->get_name_by_bts($bts_id) as $key=>$val){
			array_push($arr,'"' . $key . '":"' . $val . '"');
		}
		$out = implode(',',$arr);
		$out = '{' . $out . '}';
