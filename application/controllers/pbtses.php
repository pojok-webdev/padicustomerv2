...
class Pbtses extends CI_Controller{
    ...
    function btses(){
        $this->check_login();
        $this->data['objs'] = $this->btstower->populate();
    ...
    function btssave(){
        ...
        echo $this->pbtstower->add($params);
    }
    function feed(){
        $term = $_GET[ "term" ];
    ...
    function index(){
        ...
        $this->data['objs'] = $this->pbtstower->populate();
        $this->data['branches'] = $this->branch->get_combo_data();
        $this->data['menuFeed'] = 'bts';
        $this->load->view('adm/btses', $this->data);        
    }
	...
	function show_bts(){
		$btses = new Btstower();
		$this->common->show_object($btses, 'bts_management', 'btses');
	}
	function entry_bts(){
	...
	function update(){
		$params = $this->input->post();
		echo $this->pbtstower->edit($params);
	}
}